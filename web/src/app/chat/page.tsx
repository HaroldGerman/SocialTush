          ...
          <div className="flex-1 space-y-1 overflow-y-auto bg-[#f4f7f8] p-3 dark:bg-[#080e19]">
            {filteredConversations.map(item => {
              ...
              return <div key={item.conversationId} onClick={() => setActiveConversation(item)} className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-3 transition-all ${activeConversation?.conversationId === item.conversationId ? 'border-[#2a968b]/40 bg-[#1b8a80]/10 dark:bg-[#10242a]' : 'border-transparent bg-white/90 hover:bg-white dark:bg-transparent dark:hover:bg-[#101827]'}`}>
                ...
                <div className="min-w-0 flex-1"><div className="flex items-center justify-between"><span className={`truncate text-xs ${hasUnread ? 'font-black text-slate-950 dark:text-white' : 'font-bold text-slate-800 dark:text-slate-200'}`}>{item.isPinned && <Bookmark className="mr-1 inline h-3 w-3 fill-[#1b8a80] text-[#1b8a80]"/>}{item.name}</span><span className="text-[9px] text-slate-400">{formatTimeAgo(item.updatedAt)}</span></div><div className="mt-1 flex items-center justify-between gap-2"><span className={`flex min-w-0 items-center gap-1 truncate text-[10px] ${hasUnread ? 'font-bold text-[#177f76] dark:text-[#55c7bb]' : 'text-slate-500 dark:text-slate-400'}`}>{item.latestMessageSenderUsername?.toLowerCase() === user?.username?.toLowerCase() && <LifonkReceipt compact read={Boolean(item.latestMessageReadReceiptVisible && item.latestMessageReadByRecipient)}/>}<span className="truncate">{preview}</span></span>{hasUnread && <span className="ml-2 min-w-4 rounded-full bg-[#177f76] px-1.5 py-0.5 text-center text-[9px] font-black text-white">{item.unreadCount}</span>}</div></div>
