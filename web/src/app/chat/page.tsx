...
import {
  Bookmark, Camera, ChevronLeft, Eye, Home, Image as ImageIcon, Info,
...
interface Conversation {
  ...
  mutedUntil?: string;
  chatTheme?: 'DEFAULT' | 'DEEP_VIOLET' | 'OCEAN' | 'FOREST' | 'NIGHT' | 'PEARL' | 'AURORA_LIGHT';
}
...
const normalizeChatThemeFromApi = (theme?: string) => theme === 'DEEP_TEAL' ? 'DEEP_VIOLET' : theme;
const normalizeChatThemeForApi = (theme?: string) => theme === 'DEEP_VIOLET' ? 'DEEP_TEAL' : theme;
...
function LifonkReceipt({ read, compact = false }: { read: boolean; compact?: boolean }) {
  return <span className={`inline-flex items-center ${compact ? 'gap-0.5' : 'gap-1'}`} aria-label={read ? 'Leído' : 'Entregado'} title={read ? 'Leído' : 'Entregado'}>
    <span className={`relative inline-flex ${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} items-center justify-center rounded-full border font-black ${read ? 'border-[#7c3aed] bg-[#7c3aed] text-white' : 'border-slate-400/80 bg-transparent text-slate-400'}`}>
      <span className={`${compact ? 'text-[7px]' : 'text-[8px]'} leading-none`}>L</span>
      <span className={`absolute -right-0.5 -top-0.5 h-1.5 w-1.5 rounded-full ${read ? 'bg-[#c4b5fd]' : 'bg-slate-400'}`}/>
    </span>
  </span>;
}
...
function ChatContent() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      const data = (response.data || []).map((item: Conversation & { chatTheme?: string }) => ({
        ...item,
        chatTheme: normalizeChatThemeFromApi(item.chatTheme) as Conversation['chatTheme'],
      }));
  ...
  const updateConversationPreferences = async (values: { notificationsMuted?: boolean; mutedUntil?: string | null; chatTheme?: string }) => {
    if (!activeConversation?.conversationId) return;
    try {
      const payload = values.chatTheme ? { ...values, chatTheme: normalizeChatThemeForApi(values.chatTheme) } : values;
      const response = await api.patch(`/chat/conversations/${activeConversation.conversationId}/preferences`, payload);
      const normalizedResponse = response.data?.chatTheme
        ? { ...response.data, chatTheme: normalizeChatThemeFromApi(response.data.chatTheme) }
        : response.data;
      setActiveConversation(previous => previous ? { ...previous, ...normalizedResponse } : previous);
      await fetchConversations();
  ...
  const chatThemeBackground: Record<string, string> = {
    DEFAULT: '',
    DEEP_VIOLET: 'radial-gradient(circle at 82% 12%, rgba(124,58,237,.16), transparent 28%), linear-gradient(150deg,#0d0a19,#150c24)',
    OCEAN: 'radial-gradient(circle at 15% 85%, rgba(139,92,246,.18), transparent 32%), linear-gradient(150deg,#0d0a19,#160f28)',
    FOREST: 'radial-gradient(circle at 75% 12%, rgba(91,33,182,.18), transparent 30%), linear-gradient(150deg,#0d0a19,#140b22)',
    NIGHT: 'linear-gradient(145deg,#0b0714,#161129)',
    PEARL: 'radial-gradient(circle at 12% 18%,rgba(124,58,237,.10),transparent 24%), radial-gradient(circle at 88% 82%,rgba(196,181,253,.14),transparent 28%), linear-gradient(145deg,#ffffff,#f7f4fd)',
    AURORA_LIGHT: 'radial-gradient(circle at 80% 8%,rgba(167,139,250,.16),transparent 24%), radial-gradient(circle at 8% 78%,rgba(196,181,253,.18),transparent 30%), radial-gradient(circle at 72% 68%,rgba(139,92,246,.12),transparent 25%), linear-gradient(155deg,#fffefd,#faf7ff 52%,#f6f3ff)',
  };
  ...
  return (
    <main className="h-[100dvh] min-h-[100dvh] select-none overflow-hidden overscroll-none bg-[#edf2f4] font-sans text-slate-800 transition-colors dark:bg-[#060a12] dark:text-slate-100 lg:p-4">
        ...
        <aside className="z-20 hidden w-60 flex-shrink-0 flex-col justify-between border-r border-slate-200 bg-white p-4 shadow-sm dark:border-[#1b293b] dark:bg-[#0d1524] lg:flex">
          <div className="space-y-6">
            <Link href="/feed" className="flex items-center gap-3 px-2 py-1">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#7c3aed] text-xl font-extrabold text-white shadow-md shadow-[#7c3aed]/20">L</div>
            ...
            <nav className="space-y-1 text-xs font-semibold">
              ...
              <Link href="/chat" className="flex items-center justify-between rounded-xl border-l-4 border-[#8b5cf6] bg-[#8b5cf6]/10 px-3 py-2.5 font-bold text-[#6d28d9] dark:text-[#c4b5fd]"><span className="flex items-center gap-3"><MessageSquare className="h-4 w-4"/>Conversaciones</span>{totalUnreadAll > 0 && <span className="rounded-full bg-[#7c3aed] px-2 py-0.5 text-[10px] text-white">{totalUnreadAll}</span>}</Link>
        ...
        <div className={`w-full flex-shrink-0 flex-col justify-between border-r border-slate-200 bg-slate-50 dark:border-[#1b293b] dark:bg-[#0b1220] md:w-80 lg:w-80 ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
          <div className="space-y-3 border-b border-slate-200 bg-white p-4 dark:border-[#1b293b] dark:bg-[#0d1524]">
            <div className="flex items-center justify-between">
              <div><p className="text-sm font-black uppercase tracking-[.24em] text-[#8b5cf6] dark:text-[#c4b5fd]">Lifonk</p><h2 className="text-lg font-extrabold tracking-tight text-slate-900 dark:text-white">Conversaciones</h2></div>
              <div className="flex items-center gap-1.5">
                ...
                <button onClick={() => setIsNewChatModalOpen(true)} className="rounded-xl border border-[#8b5cf6]/40 bg-[#8b5cf6]/10 p-2 text-[#7c3aed] dark:text-[#c4b5fd]" title="Nueva conversación"><Plus className="h-4 w-4"/></button>
                <button onClick={() => setIsNewGroupModalOpen(true)} className="rounded-xl border border-[#8b5cf6]/40 bg-[#8b5cf6]/10 p-2 text-[#7c3aed] dark:text-[#c4b5fd]" title="Crear grupo"><Users className="h-4 w-4"/></button>
              </div>
            </div>
            <label className="relative block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input value={conversationSearch} onChange={event => setConversationSearch(event.target.value)} placeholder="Buscar conversaciones…" className="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-10 pr-3 text-xs outline-none focus:border-[#8b5cf6] dark:border-[#203047] dark:bg-[#09111d] dark:text-white"/></label>
            {recentDirectConversations.length > 0 && <div><p className="mb-2 text-[10px] font-black uppercase tracking-wider text-slate-400">Recientes</p><div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none]">{recentDirectConversations.map(item => <button key={item.conversationId} onClick={() => setActiveConversation(item)} className="w-12 shrink-0 text-center"><UserAvatar avatarUrl={item.avatarUrl} name={item.name} className="mx-auto h-10 w-10 rounded-full border border-[#8b5cf6]/60 text-[10px]"/><span className="mt-1 block truncate text-[9px] font-semibold text-slate-600 dark:text-slate-300">{item.name.split(' ')[0]}</span></button>)}</div></div>}
            <div className="flex rounded-2xl border border-slate-200 bg-slate-100 p-1 text-[11px] font-bold dark:border-[#203047] dark:bg-[#09111d]">
              <button onClick={() => setActiveTab('directos')} className={`flex-1 rounded-xl py-2 transition-all ${activeTab === 'directos' ? 'bg-[#7c3aed] text-white shadow-lg shadow-[#7c3aed]/15' : 'text-slate-600 dark:text-slate-400'}`}>Directos ({conversations.filter(item => !item.isGroup).length})</button>
              <button onClick={() => setActiveTab('circulos')} className={`flex-1 rounded-xl py-2 transition-all ${activeTab === 'circulos' ? 'bg-[#7c3aed] text-white shadow-lg shadow-[#7c3aed]/15' : 'text-slate-600 dark:text-slate-400'}`}>Círculos</button>
            </div>
            <div className="flex gap-2 overflow-x-auto [scrollbar-width:none]">{([['todos','Todas'],['noleidos','No leídas'],['ancladas','Ancladas'],['recientes','Recientes']] as const).map(([value,label]) => <button key={value} onClick={() => setFilterCategory(value)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-[10px] font-bold ${filterCategory === value ? 'border-[#8b5cf6] bg-[#7c3aed] text-white' : 'border-slate-200 text-slate-500 dark:border-[#203047] dark:text-slate-400'}`}>{label}</button>)}</div>
          </div>
          ...
          <div className="flex-1 space-y-1 overflow-y-auto bg-[#f4f7f8] p-3 dark:bg-[#080e19]">
            {filteredConversations.map(item => {
              ...
              return <div key={item.conversationId} onClick={() => setActiveConversation(item)} className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-3 transition-all ${activeConversation?.conversationId === item.conversationId ? 'border-[#8b5cf6]/40 bg-[#8b5cf6]/10 dark:bg-[#10242a]' : 'border-transparent bg-white/90 hover:bg-white dark:bg-transparent dark:hover:bg-[#101827]'}`}>
                {item.isGroup ? <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#7c3aed] text-white"><Users className="h-5 w-5"/></div> : <UserAvatar avatarUrl={item.avatarUrl} name={item.name} className="h-10 w-10 rounded-full text-xs"/>}
                <div className="min-w-0 flex-1"><div className="flex items-center justify-between"><span className={`truncate text-xs ${hasUnread ? 'font-black text-slate-950 dark:text-white' : 'font-bold text-slate-800 dark:text-slate-200'}`}>{item.isPinned && <Bookmark className="mr-1 inline h-3 w-3 fill-[#8b5cf6] text-[#8b5cf6]"/>}{item.name}</span><span className="text-[9px] text-slate-400">{formatTimeAgo(item.updatedAt)}</span></div><div className="mt-1 flex items-center justify-between gap-2"><span className={`flex min-w-0 items-center gap-1 truncate text-[10px] ${hasUnread ? 'font-bold text-[#7c3aed] dark:text-[#c4b5fd]' : 'text-slate-500 dark:text-slate-400'}`}>{item.latestMessageSenderUsername?.toLowerCase() === user?.username?.toLowerCase() && <LifonkReceipt compact read={Boolean(item.latestMessageReadReceiptVisible && item.latestMessageReadByRecipient)}/>}<span className="truncate">{preview}</span></span>{hasUnread && <span className="ml-2 min-w-4 rounded-full bg-[#7c3aed] px-1.5 py-0.5 text-center text-[9px] font-black text-white">{item.unreadCount}</span>}</div></div>
        ...
        <div className={`relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-slate-50 dark:bg-[#080e19] ${!activeConversation ? 'hidden md:flex' : 'flex'}`}>
          {activeConversation ? <>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_80%_8%,rgba(124,58,237,.08),transparent_26%),radial-gradient(circle_at_18%_70%,rgba(139,92,246,.06),transparent_30%)] p-4 dark:bg-[radial-gradient(circle_at_82%_8%,rgba(139,92,246,.10),transparent_26%),radial-gradient(circle_at_12%_75%,rgba(91,33,182,.24),transparent_32%),linear-gradient(155deg,#0a0714,#100c1f_56%,#0c0a19)] md:p-6" style={activeConversation.chatTheme && activeConversation.chatTheme !== 'DEFAULT' ? { backgroundImage: chatThemeBackground[activeConversation.chatTheme] } : undefined}>
              ...
              {otherUserTyping && <div className="flex items-center gap-2 py-1 text-xs font-bold text-[#8b5cf6]"><span className="h-2 w-2 animate-bounce rounded-full bg-[#8b5cf6]"/>{activeConversation.name} está escribiendo…</div>}
            ...
            {selectedAttachment && attachmentPreviewUrl && <div className="mx-4 mb-2 flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-[#26364c] dark:bg-[#101827]">{selectedAttachment.type.startsWith('image/') && <img src={attachmentPreviewUrl} alt="Vista previa" className="h-16 w-16 rounded-xl object-cover"/>}{selectedAttachment.type.startsWith('video/') && <video src={attachmentPreviewUrl} className="h-16 w-24 rounded-xl object-cover" muted playsInline/>}{selectedAttachment.type.startsWith('audio/') && <audio src={attachmentPreviewUrl} controls className="h-10 flex-1"/>}<div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{selectedAttachment.name}</p><p className="text-[10px] text-slate-500">{(selectedAttachment.size / 1024 / 1024).toFixed(2)} MB</p></div><button disabled={isSendingAttachment} onClick={clearSelectedAttachment} className="rounded-lg p-2 text-slate-500"><X className="h-4 w-4"/></button>{selectedAttachment.type.startsWith('image/') && !activeConversation.isGroup && <button type="button" onClick={() => setViewOnceSelected(value => !value)} className={`flex w-full items-center justify-between rounded-xl border px-3 py-2 text-left text-[11px] font-bold sm:w-auto ${viewOnceSelected ? 'border-[#8b5cf6] bg-[#8b5cf6]/12 text-[#7c3aed] dark:text-[#c4b5fd]' : 'border-slate-200 dark:border-[#26364c]'}`}><span className="flex items-center gap-2"><Eye className="h-4 w-4"/>Ver una vez</span><span className={`ml-4 h-5 w-9 rounded-full p-0.5 transition ${viewOnceSelected ? 'bg-[#7c3aed]' : 'bg-slate-300 dark:bg-[#334155]'}`}><span className={`block h-4 w-4 rounded-full bg-white transition-transform ${viewOnceSelected ? 'translate-x-4' : ''}`}/></span></button>}{selectedAttachment.type.startsWith('audio/') && <button disabled={isSendingAttachment} onClick={() => void sendCurrentMessage()} className="ml-auto inline-flex items-center gap-2 rounded-xl bg-[#7c3aed] px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{isSendingAttachment ? 'Enviando…' : 'Enviar audio'}<Send className="h-3.5 w-3.5"/></button>}</div>}

            <div className="flex shrink-0 items-end gap-2 border-t border-slate-200 bg-white p-3 shadow-[0_-8px_24px_rgba(15,23,42,.04)] dark:border-[#1b293b] dark:bg-[#0d1524]" style={{ paddingBottom: 'calc(.75rem + env(safe-area-inset-bottom))' }}>
              <button onClick={() => attachmentInputRef.current?.click()} className="mb-0.5 rounded-xl border border-slate-200 bg-slate-100 p-2.5 text-slate-600 dark:border-[#26364c] dark:bg-[#162033] dark:text-slate-300"><Plus className="h-4 w-4"/></button>
      ...
      {isNewChatModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm"><div className="w-full max-w-sm space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-[#26364c] dark:bg-[#111a29]"><div className="flex items-center justify-between"><h3 className="text-sm font-bold">Nueva conversación</h3><button onClick={() => setIsNewChatModalOpen(false)}><X className="h-4 w-4"/></button></div><form onSubmit={handleCreateNewChat} className="space-y-3"><input value={searchUsername} onChange={event => setSearchUsername(event.target.value)} placeholder="Nombre de usuario" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs outline-none dark:border-[#26364c] dark:bg-[#09111d]" required/><button className="w-full rounded-xl bg-[#7c3aed] py-2.5 text-xs font-bold text-white">Iniciar conversación</button></form></div></div>}
      {isNewGroupModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm"><div className="w-full max-w-sm space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-[#26364c] dark:bg-[#111a29]"><div className="flex items-center justify-between"><h3 className="text-sm font-bold">Crear Círculo o Nodo</h3><button onClick={() => setIsNewGroupModalOpen(false)}><X className="h-4 w-4"/></button></div><form onSubmit={handleCreateNewGroup} className="space-y-3"><input value={groupName} onChange={event => setGroupName(event.target.value)} placeholder="Nombre del grupo" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-[#26364c] dark:bg-[#09111d]" required/><input value={groupParticipants} onChange={event => setGroupParticipants(event.target.value)} placeholder="alex, kathely" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-[#26364c] dark:bg-[#09111d]" required/><button className="w-full rounded-xl bg-[#7c3aed] py-2.5 text-xs font-bold text-white">Crear Círculo</button></form></div></div>}
...
export default function ChatPage() {
  return <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-[#f8fafc] dark:bg-[#070b13]"><div className="flex animate-pulse flex-col items-center gap-3"><div className="h-10 w-10 rounded-xl bg-[#7c3aed]"/><span className="text-sm font-semibold text-[#7c3aed] dark:text-[#c4b5fd]">Cargando conversaciones…</span></div></div>}><ChatContent/></Suspense>;
}