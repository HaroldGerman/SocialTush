...
export default function CircleDetailPage() {
  ...
  const toggleLike = async (postId: string) => {
    ...
    } catch (error: any) { setActionError(error.response?.data?.message || 'No se pudo actualizar la resonancia.'); }
  };
  ...
  const deletePost = async (postId: string) => {
    if (!window.confirm('¿Eliminar esta contribución?')) return;
    try {
      await api.delete(`/posts/${postId}`);
      setPosts(current => current.filter(post => post.postId !== postId));
    } catch (error: any) { setActionError(error.response?.data?.message || 'No se pudo eliminar la contribución.'); }
  };
  ...
  return <div className="min-h-screen bg-[#f4f6f9] text-slate-800 dark:bg-[#090d16] dark:text-slate-100">
    ...
    <main className="mx-auto grid max-w-5xl grid-cols-1 gap-7 px-4 py-8 lg:grid-cols-3"><div className="space-y-5 lg:col-span-2">
      ...
      {circle.isMember ? <PostForm circle={circle} postText={postText} setPostText={setPostText} selectedFile={selectedFile} previewUrl={previewUrl} selectFile={selectFile} fileInputRef={fileInputRef} isSubmitting={isSubmitting} onSubmit={handlePostSubmit} /> : <div className="rounded-3xl border border-slate-200 bg-white p-5 text-center text-sm text-slate-500 dark:border-slate-800 dark:bg-[#0f172a]">Entra al círculo para contribuir.</div>}
      {posts.length === 0 ? <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center dark:border-slate-800 dark:bg-[#0f172a]"><MessageCircle className="mx-auto h-10 w-10 text-teal-400" /><h3 className="mt-2 font-bold">Aún no hay contribuciones</h3><p className="text-xs text-slate-500">Sé la primera persona en compartir una contribución.</p></div> : posts.map(post => <PostCard key={post.postId} post={post} own={user?.userId === post.userId} commentsOpen={Boolean(expandedComments[post.postId])} comments={comments[post.postId] || []} commentText={commentInputs[post.postId] || ''} setCommentText={(value: string) => setCommentInputs(current => ({ ...current, [post.postId]: value }))} onLike={() => toggleLike(post.postId)} onSave={() => toggleSave(post.postId)} onComments={() => toggleComments(post.postId)} onComment={(event: React.FormEvent) => addComment(post.postId, event)} onDelete={() => deletePost(post.postId)} />)}
    </div><aside><div className="rounded-3xl border border-slate-200 bg-white p-5 text-sm dark:border-slate-800 dark:bg-[#0f172a]"><h3 className="font-bold">Privacidad</h3><p className="mt-2 text-xs text-slate-500">{circle.visibility === 'PUBLIC' ? 'El contenido de este círculo es público.' : 'Solo integrantes autorizados pueden ver este círculo.'}</p></div></aside></main>
  </div>;
}

function PostForm({ circle, postText, setPostText, selectedFile, previewUrl, selectFile, fileInputRef, isSubmitting, onSubmit }: any) {
  return <form onSubmit={onSubmit} className="space-y-3 rounded-3xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#0f172a]"><h2 className="text-xs font-bold uppercase tracking-wide">Contribuir en {circle.name}</h2><textarea rows={3} value={postText} onChange={event => setPostText(event.target.value)} placeholder="¿Qué quieres compartir?" className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm outline-none focus:border-teal-500 dark:border-slate-700 dark:bg-slate-900" />{selectedFile && <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">{selectedFile.type.startsWith('video/') ? <video src={previewUrl} controls playsInline className="max-h-72 w-full bg-black object-contain" /> : <img src={previewUrl} alt="Vista previa" className="max-h-72 w-full object-contain" />}<button type="button" aria-label="Quitar archivo" onClick={() => selectFile()} className="absolute right-2 top-2 rounded-full bg-black/70 p-2 text-white"><X className="h-4 w-4" /></button><p className="p-2 text-xs text-slate-500">{selectedFile.name} · {(selectedFile.size / 1024 / 1024).toFixed(1)} MB</p></div>}<div className="flex justify-between"><input ref={fileInputRef} hidden type="file" accept="image/*,video/*" onChange={event => selectFile(event.target.files?.[0])} /><button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 text-xs font-bold text-teal-700 dark:text-teal-300"><ImageIcon className="h-4 w-4" />Foto/Video</button><button disabled={isSubmitting || (!postText.trim() && !selectedFile)} className="flex items-center gap-2 rounded-xl bg-teal-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-50"><Send className="h-4 w-4" />{isSubmitting ? 'Contribuyendo…' : 'Contribuir'}</button></div></form>;
}
