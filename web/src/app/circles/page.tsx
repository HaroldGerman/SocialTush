...
export default function CirclesPage() {
  ...
  const fetchCircles = async () => {
    ...
    try {
      const [allRes, mineRes] = await Promise.all([
        api.get('/circles'),
        user ? api.get('/circles/mine').catch(() => ({ data: [] })) : Promise.resolve({ data: [] })
      ]);
  ...
  const handleJoinToggle = async (circle: CircleItem) => {
    try {
      ...
      alert(err.response?.data?.message || 'Error al cambiar membresía del círculo');
    }
  };
  ...
  const handleCreateCircle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Error al crear círculo');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9] text-slate-800 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link href="/feed" className="flex items-center gap-2 group">
              ...
              <span className="font-bold text-xl tracking-tight text-teal-950">
          ...
          {/* Navigation Links */}
          <nav className="hidden md:flex items-center gap-1 bg-teal-50/70 p-1 rounded-2xl border border-teal-100">
            <Link href="/feed" className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-teal-700 rounded-xl transition-colors">
              Inicio
            </Link>
            <Link href="/circles" className="px-4 py-2 text-xs font-bold text-teal-800 bg-white shadow-xs rounded-xl transition-colors flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-3">
            ...
            {user && (
              <Link href={`/profile/${user.username}`} className="h-9 w-9 rounded-xl bg-teal-100 flex items-center justify-center text-teal-800 font-bold text-xs border border-teal-200">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-teal-100 shadow-sm">
          {/* Tabs */}
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl text-xs bg-slate-50 border border-slate-200 focus:outline-none focus:border-teal-500 focus:bg-white transition-all"
            />
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(n => (
              <div key={n} className="h-56 bg-white rounded-3xl animate-pulse p-6 border border-slate-100" />
            ))}
          </div>
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-teal-100 space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-800">No se encontraron Círculos</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCircles.map(circle => (
              <div
                key={circle.id}
                className="bg-white rounded-3xl p-6 border border-teal-100/80 shadow-xs hover:shadow-md hover:border-teal-200 transition-all flex flex-col justify-between group"
              >
                <div className="space-y-4">
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="h-12 w-12 rounded-2xl bg-teal-100 border border-teal-200 flex items-center justify-center text-teal-800 font-black text-lg group-hover:scale-105 transition-transform">
                        {circle.name.charAt(0)}
                      </div>
                      <div>
                        <Link href={`/circles/${circle.slug}`} className="font-bold text-sm text-slate-900 hover:text-teal-700 transition-colors line-clamp-1">
                          {circle.name}
                        </Link>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 animate-scale-in">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="text-base font-bold text-slate-900">Crear Nuevo Círculo</h3>
              <button
            ...
            <form onSubmit={handleCreateCircle} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl text-xs border border-slate-200 focus:outline-none focus:border-teal-500"
                />
              </div>
