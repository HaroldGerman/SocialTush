...
export default function ReelsPage() {
  ...
  const fetchPage = useCallback(async (nextPage: number, replace = false) => {
    ...
    } catch (requestError) { console.error('No se pudieron cargar los Reels:', requestError); setError('No se pudieron cargar los Reels.'); }
  ...
  const handleLike = async (postId: string) => { try { const res = await api.post(`/likes/${postId}`); setReels(old => old.map(reel => reel.postId === postId ? { ...reel, hasLiked: res.data.liked, likesCount: res.data.count } : reel)); setError(''); } catch (requestError) { console.error(requestError); setError('No se pudo actualizar el Me gusta.'); } };
  const handleSave = async (postId: string) => { try { const res = await api.post(`/posts/${postId}/save`); setReels(old => old.map(reel => reel.postId === postId ? { ...reel, isSaved: res.data.saved } : reel)); setError(''); } catch (requestError) { console.error(requestError); setError('No se pudo actualizar Guardados.'); } };
  ...
  const openComments = async (reel: ReelDto) => { setCommentsFor(reel); setComments([]); setCommentsLoading(true); setError(''); try { const res = await api.get(`/comments/${reel.postId}`); setComments(res.data || []); } catch (requestError) { console.error(requestError); setError('No se pudieron cargar los comentarios.'); } finally { setCommentsLoading(false); } };
  ...
  const deleteReel = async () => { if (!deleteTarget) return; try { await api.delete(`/posts/${deleteTarget.postId}`); setReels(old => old.filter(reel => reel.postId !== deleteTarget.postId)); setActiveIndex(index => Math.max(0, Math.min(index, reels.length - 2))); setDeleteTarget(null); setError(''); } catch (requestError) { console.error(requestError); setError('No se pudo eliminar el Reel.'); } };
  const share = async (reel: ReelDto) => { const url = `${window.location.origin}/post/${reel.postId}`; try { if (navigator.share) await navigator.share({ title: `Reel de @${reel.username}`, url }); else { await navigator.clipboard.writeText(url); setError('Enlace copiado.'); } } catch (shareError) { if ((shareError as DOMException).name !== 'AbortError') setError('No se pudo compartir el enlace.'); } };

  if (loading || isLoading) return <div className="flex min-h-screen items-center justify-center bg-[#090d16] text-teal-400">Cargando Reels…</div>;
  if (!reels.length) return <div className="flex min-h-screen flex-col items-center justify-center bg-[#090d16] p-6 pb-20 text-white"><Sparkles className="mb-4 h-12 w-12 text-teal-400"/><h3 className="text-lg font-extrabold">No hay Reels disponibles</h3><p className="mb-6 text-xs text-slate-400">Los Reels publicados aparecerán aquí.</p>{error && <p className="mb-3 text-xs text-rose-400">{error}</p>}<button onClick={() => void fetchPage(0, true)} className="rounded-xl bg-teal-700 px-5 py-2.5 text-xs font-bold">Reintentar</button><MobileBottomBar/></div>;
  ...
  return <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#090d16] pb-16 text-white md:pb-0">
    <div className="absolute left-4 right-4 top-[max(1rem,env(safe-area-inset-top))] z-40 mx-auto flex max-w-md items-center justify-between"><button aria-label="Volver" onClick={() => router.back()} className="rounded-full border border-white/10 bg-black/40 p-2.5"><ArrowLeft className="h-5 w-5"/></button><b className="text-sm uppercase tracking-wider text-teal-400">Reels</b><button aria-label={muted ? 'Activar sonido' : 'Silenciar'} onClick={() => setMuted(value => !value)} className="rounded-full border border-white/10 bg-black/40 p-2.5">{muted ? <VolumeX className="h-5 w-5"/> : <Volume2 className="h-5 w-5"/>}</button></div>
    ...
    <div ref={containerRef} onScroll={handleScroll} className="relative h-[100dvh] w-full max-w-md snap-y snap-mandatory overflow-y-scroll scrollbar-none">
      {reels.map((reel, index) => <article key={reel.postId} className="relative flex h-full w-full snap-start items-center justify-center overflow-hidden bg-slate-900">
        {reel.mediaTypes?.[0] === 'VIDEO' && reel.mediaUrls?.[0] ? <video ref={element => { videoRefs.current[reel.postId] = element; }} src={reel.mediaUrls[0]} loop muted={muted} playsInline preload={Math.abs(index - activeIndex) <= 1 ? 'auto' : 'metadata'} onClick={() => togglePlayback(reel)} className="h-full w-full object-cover"/> : <div className="p-8 text-center text-sm text-slate-400">Este Reel no contiene un video válido.</div>}
        ...
        <div className="absolute bottom-24 right-4 z-30 flex flex-col items-center gap-5">{user?.userId === reel.userId && <button aria-label="Opciones del Reel" onClick={() => setDeleteTarget(reel)} className="rounded-full border border-white/10 bg-black/40 p-3"><MoreVertical className="h-6 w-6"/></button>}<button aria-label="Me gusta" onClick={() => void handleLike(reel.postId)} className="flex flex-col items-center gap-1"><span className="rounded-full border border-white/10 bg-black/40 p-3"><Heart className={`h-6 w-6 ${reel.hasLiked ? 'fill-rose-500 text-rose-500' : ''}`}/></span><b className="text-[11px]">{reel.likesCount}</b></button><button aria-label="Comentarios" onClick={() => void openComments(reel)} className="flex flex-col items-center gap-1"><span className="rounded-full border border-white/10 bg-black/40 p-3"><MessageCircle className="h-6 w-6"/></span><b className="text-[11px]">{reel.commentsCount}</b></button><button aria-label="Guardar" onClick={() => void handleSave(reel.postId)} className="rounded-full border border-white/10 bg-black/40 p-3"><Bookmark className={`h-6 w-6 ${reel.isSaved ? 'fill-teal-400 text-teal-400' : ''}`}/></button><button aria-label="Compartir" onClick={() => void share(reel)} className="rounded-full border border-white/10 bg-black/40 p-3"><Share2 className="h-6 w-6"/></button></div>
    ...
    {commentsFor && <div className="fixed inset-0 z-[80] flex items-end bg-black/60 md:items-center md:justify-center" onMouseDown={event => { if (event.target === event.currentTarget) setCommentsFor(null); }}><section className="flex max-h-[75dvh] w-full flex-col rounded-t-3xl bg-white text-slate-900 md:max-w-lg md:rounded-3xl"><header className="flex items-center justify-between border-b p-4"><b>Comentarios ({commentsFor.commentsCount})</b><button aria-label="Cerrar" onClick={() => setCommentsFor(null)}><X className="h-5 w-5"/></button></header><div className="min-h-32 flex-1 space-y-3 overflow-y-auto p-4">{commentsLoading ? <p className="text-sm text-slate-500">Cargando…</p> : comments.length ? comments.map(comment => <div key={comment.commentId} className="flex gap-3"><UserAvatar avatarUrl={comment.avatarUrl} name={comment.displayName || comment.username} className="h-8 w-8 rounded-full text-[10px]"/><div><p className="text-xs"><b>@{comment.username}</b> <span className="text-slate-400">· {formatLocalTimestamp(comment.createdAt)}</span></p><p className="break-words text-sm">{comment.content}</p></div></div>) : <p className="text-center text-sm text-slate-500">Sé el primero en comentar.</p>}</div><form onSubmit={sendComment} className="flex gap-2 border-t p-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><input value={commentText} onChange={event => setCommentText(event.target.value)} placeholder="Escribe un comentario…" className="min-w-0 flex-1 rounded-xl border px-3 py-2 text-sm"/><button disabled={!commentText.trim()} className="rounded-xl bg-teal-700 px-4 text-sm font-bold text-white disabled:opacity-50">Enviar</button></form></section></div>}
    {deleteTarget && <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-4"><section className="w-full max-w-sm rounded-2xl bg-white p-5 text-slate-900"><h2 className="font-bold">¿Eliminar este Reel?</h2><p className="mt-1 text-sm text-slate-500">Esta acción no se puede deshacer.</p>{error && <p className="mt-3 text-xs text-rose-600">{error}</p>}<div className="mt-5 flex justify-end gap-2"><button onClick={() => { setDeleteTarget(null); setError(''); }} className="rounded-xl px-4 py-2 text-sm">Cancelar</button><button onClick={() => void deleteReel()} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white">Eliminar</button></div></section></div>}
  </div>;
}
