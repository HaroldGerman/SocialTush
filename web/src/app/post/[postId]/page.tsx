...
export default function PostDetailPage() {
  ...
  const load = async () => { setLoading(true); try { const res = await api.get(`/posts/${postId}`); setPost(res.data); setError(''); } catch (requestError: any) { console.error(requestError); setError(requestError.response?.status === 403 ? 'No tienes acceso a esta contribución.' : requestError.response?.status === 404 ? 'Contribución no encontrada.' : 'No se pudo cargar la contribución.'); } finally { setLoading(false); } };
  useEffect(() => { if (postId) void load(); }, [postId]);
  const like = async () => { try { const res = await api.post(`/likes/${postId}`); setPost((old: any) => ({ ...old, hasLiked: res.data.liked, likesCount: res.data.count })); } catch (requestError) { console.error(requestError); setError('No se pudo actualizar la resonancia.'); } };
