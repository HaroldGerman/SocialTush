...
export default function NotificationBell() {
  ...
  const dismiss = (item: Notification, event: React.MouseEvent) => { event.stopPropagation(); void markRead(item); };
  ...
  return <div className="relative"><button onClick={() => setIsOpen(value => !value)} className="relative rounded-full p-2.5 text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Notificaciones"><Bell className="h-5 w-5"/>{unreadCount > 0 && <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-extrabold text-white">{unreadCount}</span>}</button>
    {isOpen && <><div className="fixed inset-0 z-40" onClick={() => setIsOpen(false)}/><section className="absolute right-0 z-50 mt-2.5 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900"><header className="flex items-center justify-between border-b border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800"><b className="text-xs">Notificaciones</b><div className="flex gap-2">{unreadCount > 0 && <button onClick={() => void markAll()} className="text-[10px] font-bold text-teal-700 dark:text-teal-300">Marcar todo</button>}<button onClick={() => void clearRead()} className="text-[10px] text-slate-500">Limpiar leídas</button></div></header><WebPushControls />{error && <button onClick={() => setError('')} className="w-full bg-rose-50 p-2 text-left text-[11px] text-rose-700 dark:bg-rose-950 dark:text-rose-200">{error}</button>}<div className="max-h-96 divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">{notifications.map(item => <article key={item.notificationId} onClick={() => void openNotification(item)} className={`flex cursor-pointer gap-3 p-3 ${item.isRead ? '' : 'bg-teal-50/60 dark:bg-teal-950/30'} hover:bg-slate-50 dark:hover:bg-slate-800`}><UserAvatar avatarUrl={item.senderAvatarUrl} name={item.senderDisplayName || item.senderUsername} className="h-9 w-9 flex-shrink-0 rounded-full text-xs"/><div className="min-w-0 flex-1"><p className="text-xs text-slate-700 dark:text-slate-200"><b>@{item.senderUsername}</b>{item.notificationType === 'LIKE_POST' && ' le dio me gusta a tu publicación.'}{item.notificationType === 'COMMENT' && ' comentó en tu publicación.'}{item.notificationType === 'FOLLOW' && ' comenzó a seguirte.'}{item.notificationType === 'FOLLOW_REQUEST' && ' te envió una solicitud de seguimiento.'}</p><time className="mt-1 block text-[10px] text-slate-400">{formatLocalTimestamp(item.createdAt)}</time>{item.notificationType === 'FOLLOW_REQUEST' && <div className="mt-2 flex gap-2"><button onClick={event => void resolveRequest(item, 'accept', event)} className="rounded-lg bg-teal-700 px-3 py-1 text-[10px] font-bold text-white">Aceptar</button><button onClick={event => void resolveRequest(item, 'reject', event)} className="rounded-lg border border-slate-300 px-3 py-1 text-[10px] dark:border-slate-600">Rechazar</button></div>}</div>{!item.isRead && <button onClick={event => dismiss(item, event)} aria-label="Marcar notificación como leída" title="Marcar como leída" className="h-6 px-1 text-slate-400 hover:text-teal-700">×</button>}</article>)}{!notifications.length && <p className="py-8 text-center text-xs text-slate-400">Sin notificaciones.</p>}</div></section></>}
  </div>;
}
