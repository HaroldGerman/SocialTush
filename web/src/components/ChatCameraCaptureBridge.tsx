...
function originalCameraInput() {
  return Array.from(document.querySelectorAll<HTMLInputElement>('input[type="file"][capture]'))
    .find(input => input.accept.includes('image/')) || null;
}

function forwardFileToChat(file: File | undefined) {
  if (!file) return false;
  const input = originalCameraInput();
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483000] flex items-end bg-black/55 p-3 backdrop-blur-sm sm:items-center sm:justify-center" onClick={() => setOpen(false)}>
      <div className="w-full rounded-[28px] border border-[#443C68]/25 bg-white p-4 shadow-2xl dark:border-[#6d628f]/50 dark:bg-[#100d19] sm:max-w-sm" onClick={event => event.stopPropagation()}>
        ...
        <input ref={photoRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={receive} />
        <input ref={videoRef} type="file" accept="video/*" capture="environment" className="hidden" onChange={receive} />
