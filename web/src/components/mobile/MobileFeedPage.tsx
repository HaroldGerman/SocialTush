...
function FeedImage({ src, onOpen, onError }: { src: string; onOpen: () => void; onError: () => void }) {
  ...
  return (
    <button type="button" onClick={onOpen} className="relative block w-full overflow-hidden bg-slate-100 dark:bg-[#09121f]" aria-label="Abrir imagen completa">
      {!ready && <div className="flex h-64 w-full items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 dark:from-[#0b1728] dark:to-[#0f1c31]"><div className="h-8 w-8 animate-spin rounded-full border-2 border-teal-500 border-t-transparent"/></div>}
      <img
        src={src}
        alt="Publicación"
        loading="eager"
      ...
      />
      {ready && <span className="absolute bottom-3 right-3 rounded-full bg-black/55 px-3 py-1.5 text-[10px] font-bold text-white backdrop-blur">Toca para ampliar</span>}
    </button>
  );
}
...
export default function MobileFeedPage() {
  ...
  const loadStories = async () => {
    try { const response = await api.get('/stories/active'); setStories(response.data || []); }
    catch { setStories([]); }
  };
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      setSearching(true);
      try { const response = await api.get(`/search?query=${encodeURIComponent(searchQuery.trim())}`); setSearchResults(response.data?.users || []); }
  ...
  const sharePost = async (post: PostData) => {
    try {
      ...
      if (navigator.share) await navigator.share({ title: 'Lifonk', text: post.caption || 'Mira esto en Lifonk', url });
  ...
  const toggleComments = async (postId: string) => {
    ...
    if (next && !comments[postId]) {
      try { const response = await api.get(`/comments/${postId}`); setComments(previous => ({ ...previous, [postId]: response.data || [] })); }
      catch { setComments(previous => ({ ...previous, [postId]: [] })); }
    }
  };
    ...
    <div className="min-h-[100dvh] bg-[#f4f7f7] pb-24 text-slate-900 dark:bg-[#07151d] dark:text-white">
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 px-4 pb-3 pt-[calc(.7rem+env(safe-area-inset-top))] backdrop-blur-xl dark:border-slate-800 dark:bg-[#0f172a]/95">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-teal-700 text-lg font-black text-white">L</div><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-teal-600 dark:text-teal-400">Lifonk</p><h1 className="text-lg font-black leading-none">Ritmo</h1></div></div>
      ...
      <main className="mx-auto w-full max-w-xl space-y-4 px-3 py-4">
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white py-3 shadow-sm dark:border-slate-800 dark:bg-[#0f172a]">
          <div className="no-scrollbar flex gap-3 overflow-x-auto px-3">
            <div className="flex w-[66px] shrink-0 flex-col items-center gap-1.5"><div className="relative">{ownStory ? <button onClick={() => setStoryIndex(ownStoryIndex)} className="h-[60px] w-[60px] rounded-full bg-gradient-to-tr from-teal-600 via-emerald-500 to-cyan-400 p-[2px]"><UserAvatar avatarUrl={ownStory.avatarUrl || user?.avatarUrl} name={ownStory.displayName || user?.displayName || user?.username} className="h-full w-full rounded-full text-xs"/></button> : <button onClick={openStoryComposer} className="flex h-[60px] w-[60px] items-center justify-center rounded-full border-2 border-dashed border-teal-500 bg-teal-50 text-teal-700 dark:bg-teal-950/30 dark:text-teal-400"><Plus className="h-5 w-5"/></button>}{ownStory && <button onClick={openStoryComposer} className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-teal-700 text-white dark:border-[#0f172a]"><Plus className="h-3.5 w-3.5"/></button>}</div><span className="w-full truncate text-center text-[10px] font-bold">Tú</span></div>
            {others.map(({ group, index }) => { const unseen = group.hasUnseenStories ?? group.stories.some(story => !story.viewedByMe); return <button key={group.userId || group.username} onClick={() => setStoryIndex(index)} className="relative flex w-[66px] shrink-0 flex-col items-center gap-1.5"><div className={`relative h-[60px] w-[60px] rounded-full p-[2.5px] ${unseen ? 'bg-gradient-to-tr from-teal-600 via-emerald-400 to-cyan-400' : 'bg-slate-300 opacity-70 dark:bg-slate-700'}`}><UserAvatar avatarUrl={group.avatarUrl} name={group.displayName} className="h-full w-full rounded-full border-2 border-white text-xs dark:border-[#0f172a]"/>{unseen && <span className="absolute right-0 top-0 h-3 w-3 rounded-full border-2 border-white bg-teal-500 dark:border-[#0f172a]"/>}</div><span className={`w-full truncate text-center text-[10px] font-bold ${unseen ? '' : 'text-slate-400'}`}>{group.displayName}</span>{unseen && <span className="absolute -bottom-2 text-[8px] font-black uppercase text-teal-500">Nuevo</span>}</button>; })}
          </div>
        </section>
        ...
        {loading ? <div className="flex h-44 items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-teal-500 border-t-transparent"/></div> : <section className="space-y-4">{posts.map(post => <article id={post.postId} key={post.postId} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#0f172a]">
          <div className="p-4 pb-3"><Link href={`/profile/${post.username}`} className="flex min-w-0 items-center gap-3"><UserAvatar avatarUrl={post.avatarUrl} name={post.displayName || post.username} className="h-10 w-10 rounded-full text-xs"/><div className="min-w-0"><p className="truncate text-sm font-extrabold">{post.displayName || post.username}</p><p className="text-[10px] text-slate-400">@{post.username} · {formatLocalTimestamp(post.createdAt)}</p></div></Link></div>
          ...
          {post.mediaUrls?.[0] && (failedImages[post.mediaUrls[0]] ? <div className="flex h-48 items-center justify-center bg-slate-100 text-slate-400 dark:bg-slate-900"><ImageIcon className="h-7 w-7"/></div> : <FeedImage src={post.mediaUrls[0]} onOpen={() => setFullscreenImage(post.mediaUrls[0])} onError={() => setFailedImages(previous => ({ ...previous, [post.mediaUrls[0]]: true }))}/>)}
      ...
      {fullscreenImage && <div className="fixed inset-0 z-[350] flex h-[100dvh] items-center justify-center bg-black p-2 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]" onClick={() => setFullscreenImage(null)}><button aria-label="Cerrar imagen" onClick={() => setFullscreenImage(null)} className="absolute right-4 top-[calc(1rem+env(safe-area-inset-top))] z-10 flex h-11 w-11 items-center justify-center rounded-full bg-black/70 text-white"><X className="h-6 w-6"/></button><img src={fullscreenImage} alt="Imagen completa" className="max-h-full max-w-full object-contain" onClick={event => event.stopPropagation()}/></div>}
    </div>
  );
}
