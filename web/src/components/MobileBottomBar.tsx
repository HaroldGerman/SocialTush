...
import { Home, Compass, Plus, MessageSquare, User, PlaySquare, Sparkles } from 'lucide-react';
...
export default function MobileBottomBar({ onOpenCreate }: MobileBottomBarProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 grid h-[68px] grid-cols-7 items-stretch border-t border-slate-200 bg-white px-1 shadow-[0_-6px_20px_rgba(15,23,42,.07)] dark:border-slate-800 dark:bg-[#0f172a] md:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <Link href="/feed" className={`${base} ${pathname === '/feed' || pathname === '/' ? active : inactive}`}>
        <Home className="h-5 w-5 shrink-0" />
        <span className="w-full truncate text-[8px] leading-none">Ritmo</span>
      </Link>

      <Link href="/pulse" className={`${base} ${pathname.startsWith('/pulse') ? active : inactive}`}>
        <PlaySquare className="h-5 w-5 shrink-0" />
        <span className="w-full truncate text-[8px] leading-none">Pulso</span>
      </Link>

      <Link href="/discover" className={`${base} ${pathname.startsWith('/discover') ? active : inactive}`} aria-label="Descubrir">
      ...
      <Link href="/chat" className={`${base} ${pathname.startsWith('/chat') ? active : inactive}`}>
        ...
        <span className="w-full truncate text-[8px] leading-none">Chat</span>
      </Link>
      ...
      <Link href={user ? ownProfilePath : '/login'} className={`${base} ${isOwnProfile ? active : inactive}`}>
        ...
        <span className="w-full truncate text-[8px] leading-none">Espacio</span>
      </Link>
    </div>
  );
}
