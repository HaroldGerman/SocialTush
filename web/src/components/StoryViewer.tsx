...
export default function StoryViewer({ groupedStories, initialUserIndex, onClose, onStoriesChange }: StoryViewerProps) {
  ...
  useEffect(()=>{setSelectedResonance(null);setProgress(0);setVideoReady(false)},[currentStory?.storyId]);
  ...
  const onVideoTime=(video:HTMLVideoElement)=>{
    ...
    if(video.currentTime>=end-.06){video.pause();handleNextStory();}
  };
